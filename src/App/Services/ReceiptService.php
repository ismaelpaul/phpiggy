...
class ReceiptService
{
    ...
    public function validateFile(?array $file)
    {
        if (!$file || $file['error'] !== UPLOAD_ERR_OK) {
            throw new ValidationException([
                'receipt' => 'Please select a file to upload.'
            ]);
        }
        ...
        if ($file['size'] > $maxFileSizeMB) {
            throw new ValidationException([
                'receipt' => 'The file size is too large. Max size is 3MB.'
            ]);
        }
        ...
        if (!in_array($clientMimeType, $allowedMimeTypes)) {
            throw new ValidationException([
                'receipt' => 'Invalid file type. Allowed types are: jpg, png, gif, pdf.'
            ]);
        }
    }
    ...
    public function upload(array $file, int $transaction)
    {
        ...
        $uploadPath = Paths::getStorageUploads() . "/" . $newFilename;
    ...
    {
        $filePath = Paths::getStorageUploads() . '/' . $receipt['storage_filename'];
